...
export class Blockchain {
    ...
    private static async _getInjectedProviderIfExistsAsync(): Promise<InjectedProvider | undefined> {
    ...
    private static async _getInjectedProviderNetworkIdIfExistsAsync(): Promise<number | undefined> {
        // If the user has an injectedWeb3 instance that is disconnected from a backing
        // Ethereum node, this call will throw. We need to handle this case gracefully
        const injectedProviderIfExists = await Blockchain._getInjectedProviderIfExistsAsync();
    ...
    public async updateProviderToInjectedAsync(): Promise<void> {
        ...
        const networkId = await Blockchain._getInjectedProviderNetworkIdIfExistsAsync();
        await this._resetOrInitializeAsync(networkId, shouldPollUserAddress, shouldUserLedgerProvider);
    }
    ...
    private async _onPageLoadInitFireAndForgetAsync(): Promise<void> {
        ...
        const networkIdIfExists = await Blockchain._getInjectedProviderNetworkIdIfExistsAsync();
